              ...
              {contacts.map((c) => {
                ...
                return (
                  <button key={c.id} type="button" onClick={() => selectable && onToggleSelect?.(c.id)}
                    ...
                    <div className="p-3.5">
                      {/* Top: Status + Department + Checkbox */}
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[10px] text-[#64748B]" style={{ fontWeight: 500 }}>{c.department === "Supply Chain Management" ? "Supply Chain" : c.department}</span>
                        </div>
                        {selectable && (
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full flex items-center justify-center text-[10px] shrink-0" style={{ backgroundColor: t.bg, color: t.text, fontWeight: 700 }}>{ini(c.name)}</div>
                        <div className="min-w-0 flex-1">
                          <p className="text-[13px] text-[#0F172A] truncate" style={{ fontWeight: 600 }}><Hl text={c.name} q={searchQuery} /></p>
                          <p className="text-[10px] text-[#64748B] truncate mt-0.5"><Hl text={c.company} q={searchQuery} /></p>
                        </div>
                      </div>
                      {/* Contact details */}
                      <div className="space-y-1 mt-2.5 pt-2.5 border-t border-[#F1F5F9]">
